...
class DispNet(L.LightningModule):
    def __init__(self,representation,nc=1,freeze=True,anisotropy=False):
        ...
        self.register_buffer('e_bias',torch.tensor(0.25)) #Ha
    ...
    def forward(self,data,training=False,calc_force=True,calc_adiv=False):
        ...
        data["e2"] = F.relu(self.enet(data["node_feats"]) + self.e_bias) + self.min_e #(N,C)
        if self.anisotropy:
        ...
        diag_mask = torch.eye(3, device=data["e2"].device) #(3,3)
        if self.anisotropy:
        ...
        num = dyad 
        denom = data["e2"][:,:,None,None,None]**2 + self.zeta[None,None,None,None,:]**2 #(N,C,3,3,G)
        alpha = num[...,None] / denom #(N,C,3,3,G)
        data["alpha"] = alpha.sum(axis=1).moveaxis(-1,1)
        data["alpha_avg"] = alpha.diagonal(dim1=-2,dim2=-1).sum(axis=-1)/3
