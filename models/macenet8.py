...
class DispNet(L.LightningModule):
    ...
    def forward(self,data,training=False,calc_force=False,calc_adiv=False):
        ...
        num = dyad * data["e"][:,:,None,None]
        denom = data["e"][:,:,None,None,None,None] + self.zeta[None,None,None,None,:]**2 #(N,C,3,3,G)
        ...
        for i in torch.unique(data["batch"]):
            ...
            c6 = c6[mask].ravel() #remove diagonal
            if training:
                c6divs = []
                for c in c6:
